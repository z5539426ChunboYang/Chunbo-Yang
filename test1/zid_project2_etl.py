...
def daily_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    daily_returns = prc.pct_change(fill_method=None)
...
def monthly_return_cal(prc):
    ...
    valid_monthly_prc = monthly_prc[monthly_counts >= 18]
    monthly_returns = valid_monthly_prc.pct_change().dropna()
...
def aj_ret_dict(tickers, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    daily_returns_dict = {ticker.lower(): daily_return_cal(read_prc_csv(ticker, start, end)) for ticker in tickers}
    monthly_returns_dict = {ticker.lower(): monthly_return_cal(read_prc_csv(ticker, start, end)) for ticker in tickers}
    ret_dict = {
