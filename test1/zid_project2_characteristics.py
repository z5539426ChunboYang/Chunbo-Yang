...
def vol_input_sanity_check(ret, cha_name, ret_freq_use: list):
    ...
    if type(ret) is not dict or set(ret.keys()) != keys:
        raise SystemExit("The input file, `ret`, must be a dictionary with two keys: 'Daily' and 'Monthly'.")

    if type(cha_name) is not str:
        raise SystemExit("`cha_name` must be a string")
    ...
    if type(ret_freq_use) is not list or not set(ret_freq_use).issubset(keys):
        raise SystemExit("`ret_freq_use` must be a list containing 'Daily', 'Monthly', both or blank.")

    print("All input passed successfully.")
    return None
